...
目标是输出能直接进入项目、结构清晰、交互统一、风格稳定、便于维护的企业后台页面和组件代码。
...
| 场景 | 子 Skill | 说明 |
|---|---|---|
| 表格列表页 / CRUD 页面 | [crud-page](crud-page/SKILL.md) | 页面结构、表格、分页、请求数据流、命名规范、逻辑分层 |
| 表单 / 弹窗 / 抽屉 | [form-modal](form-modal/SKILL.md) | Form 校验、Modal/Drawer 使用、按钮反馈、状态管理 |
| 父子组件数据绑定 | [component-binding](component-binding/SKILL.md) | Props/Emits/v-model 模式、弹窗表单绑定、表格+查询绑定 |
| 响应式自适应布局 | [responsive-layout](responsive-layout/SKILL.md) | flex/grid 布局、断点适配、自适应规则 |
| 设计规范与 Token | [design-tokens](design-tokens/SKILL.md) | 间距、圆角、颜色、按钮等视觉约束 |
| 数据库运维页面专项 | [db-ops-pages](db-ops-pages/SKILL.md) | 工单、审批流、SQL 审核、环境区分、高风险操作 |
...
这是一个**企业后台开发 Skill**。
它追求的是：功能清晰、交互统一、风格稳定、维护简单、交付高效、响应式可用。
不是做花哨网页，也不是做通用 demo。
