...
func GetClusterDBByUsername(username string) (data []*models.SgDBOptions, err error) {
	sqlDBTotalStr := fmt.Sprintf(`select count(db.name) from database_database as db,database_servicegroup as sg where db.servicegroup_id=sg.id and db.rd_user like CONCAT('%s','%s','%s')`, "%,", username, ",%")
	sqlSGTotalStr := fmt.Sprintf(`select count(db.name) from database_database as db,database_servicegroup as sg where db.servicegroup_id=sg.id and db.rd_user like CONCAT('%s','%s','%s') group by sg.id`, "%,", username, ",%")
	var dbCnt int64
	var sgCnt int64
	if err = db.Get(&dbCnt, sqlDBTotalStr); err != nil {
		return nil, err
	}
	if err := db.Get(&sgCnt, sqlSGTotalStr); err != nil {
		return nil, err
	}

	data = make([]*models.SgDBOptions, 0, sgCnt)
	dbdata := make([]*models.SgDBOptionsChild, 0, dbCnt)
	sqlSGStr := fmt.Sprintf(`select sg.id sg_id,IFNULL(sg.name,'') sg_name from database_database as db,database_servicegroup as sg where db.servicegroup_id=sg.id and db.rd_user like CONCAT('%s','%s','%s') group by sg.id`, "%,", username, ",%")
	sqlDBStr := fmt.Sprintf(`select db.id db_id,db.name db_name,sg.id as sg_id from database_database as db,database_servicegroup as sg where db.servicegroup_id=sg.id and db.rd_user like CONCAT('%s','%s','%s')`, "%,", username, ",%")
	if err = db.Select(&data, sqlSGStr); err != nil {
		return nil, err
	}
	if err = db.Select(&dbdata, sqlDBStr); err != nil {
		return nil, err
	}
...
func GetUsernameHistoryQuery(rqUser *models.RequestGetUserHistoryQueryHandler) (data *models.RpsQueryHistoryData, err error) {
	...
	if rqUser.Table != "" {
		sqlUsernameTotalStr = fmt.Sprintf("%s and (db_name='%s' or tables='%s')", sqlUsernameTotalStr, rqUser.Table, rqUser.Table)
		sqlDBStr = fmt.Sprintf("%s and (db_name='%s' or tables='%s')", sqlDBStr, rqUser.Table, rqUser.Table)
	}
	var userQueryCount int64
	if err := db.Get(&userQueryCount, sqlUsernameTotalStr, rqUser.Username); err != nil {
		return nil, err
	}
	if userQueryCount < 1 {
		err = ErrorNoRows
	...
	rows, err := db.Query(sqlDBStr, rqUser.Username)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func ClusterGetDBTableInfoHandler(tbInfo *models.RequestGetDBTableInfo) (data *models.TableStructureData, err error) {
	...
	if tbInfo.Type == "table_structure" {
		sqlDBStr := fmt.Sprintf("show create table %s.%s ;", mysqlDsn.DBName, tbInfo.Table)
		if err = itemDsn.Select(&data.TS, sqlDBStr); err != nil {
			return nil, err
		}
	} else if tbInfo.Type == "table_base" {
		sqlDBStr := fmt.Sprintf(`select TABLE_NAME as '表名', TABLE_TYPE as '表类型', ENGINE as '引擎', ROW_FORMAT as '行格式', TABLE_ROWS as '表行数(估算值)', round(DATA_LENGTH/1024, 2) as '数据大小(KB)', round(INDEX_LENGTH/1024, 2) as '索引大小(KB)', TABLE_COLLATION as '字符集校验规则', TABLE_COMMENT as '备注', CREATE_TIME as '创建时间' from information_schema.tables where table_schema='%s' and table_name='%s'`, mysqlDsn.DBName, tbInfo.Table)
		if itemDsn.Select(&data.TB, sqlDBStr) != nil {
			return nil, err
		}
	} else {
