...
# 检查mysql没有了是否为空
{% if not salt['file.directory_exists']('/data/mysql_[[.Port]]/') %}
init_mysql_dir:
  cmd.run:
    - name: mkdir -p /data/mysql_[[.Port]]/{data,logs,run,tmp,audit} && chown -R mysql.mysql /data/mysql_[[.Port]]/
{% else %}
failure:
  test.fail_without_changes:
...
mysql_log_cmd:
  file.managed:
    - name: /data/mysql_[[.Port]]/logs/mysqld.log
    - user: mysql
    - group: mysql
    - mode: 0755
    - unless: test -f /data/mysql_[[.Port]]/logs/mysqld.log
...
mysql_cnf:
  file.managed:
    - name: /data/mysql_[[.Port]]/my_[[.Port]].cnf
    - source: salt://[[.Host]]/mysql_[[.Port]]/my_[[.Port]].cnf
    - user: mysql
    - group: mysql
    - mode: 0755
...
async_init_mysql:
  file.managed:
    - name: /tmp/init_mysql_[[.Port]].sh
    - source: salt://[[.Host]]/mysql_[[.Port]]/init_mysql_[[.Port]].sh
  cmd.run:
    - name: /bin/sh /tmp/init_mysql_[[.Port]].sh
    - onlyif: test -z "$(ls -A /data/mysql_[[.Port]]/data/)"
    - require:
      - file: mysql_cnf
      - cmd: init_mysql_dir
...
start_mysql:
  cmd.run:
    - name: /usr/local/mysql{{ mysql_version }}/bin/mysqld_safe --defaults-file=/data/mysql_[[.Port]]/my_[[.Port]].cnf &
    - user: mysql
    - bg: True
    - unless: ps -ef|grep -v 'grep' |grep [[.Port]]
    - require:
...
update_passwd:
  ...
  cmd.run:
    - name: sleep 15 && /usr/local/mysql{{ mysql_version }}/bin/mysql -uroot -p'' --connect-expired-password -S /data/mysql_[[.Port]]/run/mysql.sock -e "set global read_only = 0; set session sql_log_bin = 0; ALTER USER 'root'@'localhost' IDENTIFIED BY '123123'; flush privileges;" && /usr/local/mysql{{ mysql_version }}/bin/mysql -uroot -p'123123' -S /data/mysql_[[.Port]]/run/mysql.sock -e "source /tmp/cuser.sql"
    - unless: test -f /data/mysql_[[.Port]]/run/mysql.sock
    - require:
      - file: update_passwd

remove_tmp_file:
  cmd.run:
    - name: rm -f /tmp/init_mysql_[[.Port]].sh /tmp/cuser.sql
